...
def credentials_path() -> str:
    ...
    mine = os.path.join(home, ".config", "harvest", "tradingview.json")
    if os.path.exists(mine):
        return mine
    # A login already saved by evotrader is worth reusing rather than asking
    # for the cookies a second time.
    theirs = os.path.join(home, ".config", "evotrader", "tradingview.json")
    return theirs if os.path.exists(theirs) else mine
